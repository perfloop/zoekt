...
func BenchmarkCaseInsensitiveRegexpPrefixKelvinFoldFallback(b *testing.B) {
	const (
		pattern = "(?i)KelvinPrefixX.*"
		size    = 1 << 20
		line    = "func generated(value int) int { return value + 1 } // ordinary source text\n"
		trigger = "kelvinprefixx ascii trigger\n"
		target  = "KelvinPrefixX unicode fallback\n"
	)
	...
	benchmarkKelvinFoldFallbackSearch(b, []byte(content.String()), pattern)
}

func benchmarkKelvinFoldFallbackSearch(b *testing.B, content []byte, pattern string) {
	b.Helper()

	searcher := searcherForTest(b, testShardBuilder(b, nil, Document{
		Name:    "regexp-prefix-kelvin-fallback.go",
		Content: content,
	}))
