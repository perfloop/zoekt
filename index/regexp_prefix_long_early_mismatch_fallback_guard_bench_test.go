...
func BenchmarkCaseInsensitiveRegexpPrefixLongEarlyMismatchFallback(b *testing.B) {
	const (
		pattern  = "(?i)Abcdefghijlmn.*"
		size     = 1 << 20
		nearMiss = "Ax"
		match    = "Abcdefghijlmn fallback match\n"
	)

	var content strings.Builder
	content.Grow(size)
	content.WriteString(strings.Repeat(nearMiss, (size-len(match))/len(nearMiss)))
	content.WriteString(strings.Repeat("x", size-content.Len()-len(match)))
	content.WriteString(match)

	benchmarkLongEarlyMismatchFallbackSearch(b, []byte(content.String()), pattern)
}

func benchmarkLongEarlyMismatchFallbackSearch(b *testing.B, content []byte, pattern string) {
	b.Helper()

	searcher := searcherForTest(b, testShardBuilder(b, nil, Document{
		Name:    "regexp-prefix-long-early-mismatch-fallback.txt",
		Content: content,
	}))
